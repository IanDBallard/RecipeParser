...
**Merged:** PR #18 (https://github.com/IanDBallard/RecipeParser/pull/18), merged 2026-09-08T06:24:32-04:00 (`252ad1d`), onto `master`. First commit `30a5382`, last plan commit `1e19cae`.

**Status: executed and landed, by subagent-driven development with a task review per task and a whole-branch review at the end. The checkboxes below were never ticked; that is bookkeeping, not an unfinished plan.** Do not read status from this file. `tests/goldens/` is now 100 tests and later workstreams already build on it: the goldens were refreshed for six new recipe fields (`9caa0f1`) and again for the citation columns (`073a6a2`, `fede401`, `d11c150`). What the execution decided against this plan's text:

- **The plan's "no production behaviour changes" constraint was deliberately broken once, and the corpus is why.** `324ea88` fixes `split_large_chunk` in `epub.py`: EPUB chapter text separates paragraphs with a single newline and never a blank line, so the blank-line split never triggered and `MAX_CHUNK_CHARS` was dead for every EPUB. `gutenberg-multi.epub` produced 7 chunks, all oversized, the largest 138,167 characters. It now produces 25, none over the limit. A real latent bug that only a real-input corpus could surface — which is the case for this suite in one commit.
- **Two fixtures did not cover what the README promised.** `TINY_JPEG` (160 bytes) never cleared `MIN_PHOTO_BYTES` (20,000), so `dual-units.epub` and `text-pages.pdf` never got an `[IMAGE: ...]` marker and the reader's image path went unexercised; rebuilt with a deterministic 120x120 noise JPEG (`b601cf5`). `legacy-photo.paprikarecipes` keeps `TINY_JPEG`, since `PaprikaReader` does not gate `photo_data` by size.
- **Task 2 corrections, ruled during execution** (`6818061`): the README credited `legacy-photo.paprikarecipes` to Project Gutenberg under a borrowed licence, but its recipe text is hand-authored — the row now says so. `phases-bakers.epub` yielded only one chunk, because the multi-phase loaf's bare-gram ingredients scored zero against `is_recipe_candidate()`'s spelled-out-unit threshold, so half the fixture's stated purpose (spec line 87) never reached a reader; fixed in the fixture, not the filter. The Gutenberg fetch is pinned to `ids=29728` rather than a relevance-ranked `search=cookery` that could silently rebuild the corpus from a different book.
- **Recordings are keyed by the work unit, not a call counter** (`67cc7b5`), so the key survives any pool size.
- **Three `TestReplay` fixtures carried a placeholder prompt sha** (`"0"*64`) that always tripped the drift warning, desensitising readers to a genuine signal; they now carry the real hash (`ce643a3`).
- **Known gap, documented in `corpus/README.md` rather than only in a comment** (`1e19cae`): the e2e goldens replay extract-stage replies recorded under `units="book"` against prompts the pipeline sends as `units="us"`. Replay is correct — recordings key off the prompt body — but the reply text is a book-units answer. The dead `--synthetic` flag was removed in the same commit.
- Deviation 2 of the Self-Review below (record mode rejecting the `dummy-key-for-tests` sentinel) rests on an assumption that does not hold in a worktree: `load_dotenv()` walks up and finds the main checkout's `.env`, so a real `GOOGLE_API_KEY` wins over the sentinel. The guard is still correct and still fires in CI, where the sentinel is the only key.
- The corpus is 768,120 bytes against its 2 MB budget, after the hero-image rebuild grew it from 676,717.
