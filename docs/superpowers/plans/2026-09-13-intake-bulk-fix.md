...
Executed by subagent-driven development: a fresh implementer per task, a task review per task, one fix round (Task 4), a whole-branch review, one fix wave and one scoped re-review. What the execution decided that this plan did not, each with what it costs if wrong:

- **The 413 uses `status.HTTP_413_CONTENT_TOO_LARGE`**, not the plan's `HTTP_413_REQUEST_ENTITY_TOO_LARGE`, which Starlette 1.0 deprecates with a warning in the test output; the new name has existed since Starlette 0.37 and the declared FastAPI floor pulls 0.40. Cost: none.
- **Task 3's RED phase was reasoned, not observed** (the implementer wrongly believed the pinned tools prevented a transient run): each of its tests patches `recipeparser.adapters.api.MAX_UPLOAD_BYTES`, and `unittest.mock.patch` on a missing attribute raises `AttributeError` deterministically. Cost: a TDD-evidence gap on one task, none to the code.
- **`read_members` gained the reader's outer `except Exception`** (a fix round on Task 4): the plan's code caught only `BadGzipFile` and `JSONDecodeError`, so a truncated gzip body would have aborted the run the docstring promised to survive. Later, `UnicodeDecodeError` is caught inside the `BadGzipFile` handler in both the script and `PaprikaReader.read_entries` — the shipping reader had the same hole, where one bad member aborts a whole import. Cost: one word in the reader, tested.
- **A `pyproject.toml` per-file-ignore** for the new test's deliberate `PaprikaReader` import (ruff's TID251 bans `recipeparser.io` imports in tests; the written archive must open through the real reader), following the four reader/golden precedents; one 130-column test line reflowed. Cost: one ignore line to maintain.
- **The script tells the truth about its count** (the whole-branch review's two Important findings): `summarise()` reports unmatched members, unmatched titles (the unit the restore's 68 is in), blank-titled entries skipped, and titles with more archive entries than rows — the last listed for a by-hand check rather than over-recovered, because a wrong extra import is a duplicate the cook must find and delete. `main()` reads the written archive back and returns 1 on a mismatch. Cost if the warn-not-recover ruling is wrong: up to nine lost recipes recovered by hand instead of by the drop.
- **Parked, one residual:** `unmatched_titles` excludes blank-titled entries while the restore counted them as one title group inside its 68, so the printed title count reads one lower than 68 when a blank-titled entry is among the unmatched; the printed blank count is beside it. Cost: a moment's arithmetic at the one live run.
- **Not taken, recorded:** the `size is None` branch in `submit_file_job` is unreachable through Starlette's multipart parser (kept as insurance); a 400 MB body is spooled to disk by Starlette and then refused with the sentence — a politeness guard, not a resource guard — so a Caddy `request_body` cap above 50 MB belongs to the Cayenne deployment work; below 1 MB the two halves' size formatting would diverge (a comment, since the ceiling is 50 MB); the reader's inner handler still lets a `TypeError` from decodable-but-malformed JSON abort a read (pre-existing shape). The plan's own code sample for Task 3 keeps the old constant name; this section is the correction.