...
**Merged:** PR #36 (https://github.com/IanDBallard/RecipeParser/pull/36), merged 2026-09-12T11:03:12Z (`4377359`), onto `master`, after its Cayenne twin [#58](https://github.com/IanDBallard/Cayenne/pull/58) had applied the migration. Container redeployed from `master` on 2026-09-12 (`docker compose up -d --build`).

**Status: executed and landed, by subagent-driven development with a task review per task and a whole-branch review at the end. The checkboxes below were never ticked; that is bookkeeping, not an unfinished plan.** Do not read status from this file. The record is Stage 6, row A, of Cayenne's `SpecificationDocumentation/ROADMAP.md`. What the execution decided against this plan's text: `stated_source` and `byline` are `repr=False`, because `build_refine_prompt` embeds the extraction's repr as the `RAW RECIPE:` body that the golden client hashes (Task 3 was blocked until that ruling); "ruff clean" means clean on touched files against the repo's 139 pre-existing findings; `tests/unit/readers/test_readers.py` joined the TID per-file-ignore list; the Paprika instant-restore path gained `_citation_from(pr)` after the whole-branch review, so a Cayenne export round-trip keeps its four columns (a defect in Task 5 Step 5); `host_of` strips a path on a scheme-less host; `_get_book_source` was deleted as dead. Stage C's script half ran from this branch on 2026-09-12: `docs/ingestion-repair/2026-09-12-stage-c/`. Follow-up outside this plan: serialise the refine raw recipe schema-independently, with one paid re-record, so a future field never invalidates the corpus.
