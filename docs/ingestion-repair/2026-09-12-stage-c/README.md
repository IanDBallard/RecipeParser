...
The script counts archive *entries*, not titles, so its number is not directly the restore's 68 — 121 members share a title with another. It also prints the title count (grouped the way `plan_restore` groups), which is the number to reconcile with the 68. An entry with a blank title is reported separately and never written. A title with more archive entries than rows — one of the 9 ambiguous groups, where an entry is a genuine lost recipe the restore could not tell apart from its sibling — is listed under its own line for a by-hand check; the script recovers none of those automatically.
