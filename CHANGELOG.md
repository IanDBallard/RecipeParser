...
- The 422 for a file the API cannot read is a sentence the client shows verbatim: `Cayenne can't read .docx files yet.`; HEIC and WebP are refused plainly: `Cayenne can't read HEIC photos yet. Share it as a JPEG instead.` (and the same sentence for WebP) — no decoder for either is in the stack.
